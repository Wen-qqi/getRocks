# getRocks

GetRocks is a collectible and crafting game where players can run their own rock shop. Purchase loot boxes to obtain a wide variety of rocks and materials, each with distinct rarity and value. Manage your rocks, sell your rocks, craft jewelry and reinvest in new loot boxes to grow your collection.

## Key Features
* **Loot Boxs:** Open rock and material loot boxes to obtain unique items
* **Inventory Management:** Manage your rocks to earn money
* **Jewelry Crafting:** Combine rocks with materials to craft unique jewelry pieces for profit
* **Collectibles:** Aim to collect every type of rock and jewelry pieces

The goal of the game is run a successful rock shop, craft exquisite jewelry, and collect a variety of unique rocks.

## How to Play
**1.** Buy loot boxes to build your rock collection  
**2.** Craft jewelry by using the rocks and purchased materials  
**3.** Display your rocks and jewelry to sell it and earn more money  
**4.** Expand your collection by buying more rocks, crafting jewelry, and selling your items   

## Getting Started
**Run the Project in Godot**
1. Install **Godot Engine** (version 4.4)
2. Clone this repository
3. Open Godot → Import Project → Select the project.godot file
4. Press Play to run thr game

## Credits
- Game developed by Wen Fan and Melissa Chen
- Built with Godot 4.4
